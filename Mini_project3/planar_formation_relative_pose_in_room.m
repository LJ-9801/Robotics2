...
for k=1:N    
    ...
    qflat=reshape(q(:,k+1),n,p);
    for j=1:p
        [isInt,dist,wp]=colcheck(robot,[qflat(:,j);0],colobj);
        % use potential function to push robot back in the wp direction
        
%         while max(isnan(dist))>0
%             qflat(:,j)=qflat(:,j)+randn(2,1)*.05;
%             [isInt,dist,wp]=colcheck(robot,[qflat(:,j);0],colobj);
%             disp([k,max(dist)]);
%         end
    end
